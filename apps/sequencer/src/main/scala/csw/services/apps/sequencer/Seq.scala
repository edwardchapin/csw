...
import csw.services.ls.OldLocationService
...
object Seq {
  ...
  private def resolve(name: String, serviceType: ServiceType): BlockingCommandServiceClient = {
    val info = Await.result(OldLocationService.resolve(ServiceId(name, serviceType)), duration)
  ...
  def resolveContainer(name: String): ContainerClient = {
    val info = Await.result(OldLocationService.resolve(ServiceId(name, ServiceType.Container)), duration)
